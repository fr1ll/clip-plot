...
# %% auto 0
__all__ = ['timm_embed_model', 'timm_transform_embed', 'timm_embed', 'get_timm_embeds']
...
def timm_transform_embed(img, model, transform) -> np.ndarray:
    '''
...
def timm_embed(imgs: torch.Tensor, model) -> np.ndarray:
    '''
    run inference on image(s) to generate an embedding
...
def get_timm_embeds(imageEngine, model_name: str, **kwargs):
    ...
    batch = []
    batch_size=16
    for i, imload in enumerate(tqdm(imageEngine, total=imageEngine.count)):
        # accumulate a batch of images
        batch += [imload]
        if i%batch_size == batch_size-1 or i == imageEngine.count-1:
            # concatenate batch of transformed images so torch can embed in parallel
            imgs = torch.cat([transform(bim.original).unsqueeze(0) for bim in batch])
            b_embs = timm_embed(imgs, model)
            for bi, bim in enumerate(batch):
                # store vectors one at a time
                embed_path = vector_dir / (clean_filename(bim.path) + ".npy")
            # if embed_path.exists() and kwargs["use_cache"]:
            #     vec = np.load(embed_path)
            # else:
                np.save(embed_path, b_embs[bi])
            # save batch of embeds to list and clear batch
            embeds += list(b_embs)
            batch = []
